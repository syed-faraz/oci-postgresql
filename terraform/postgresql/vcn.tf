...
resource "oci_core_route_table" "pg_rt" {
...
resource "oci_core_security_list" "pg_sl" {
  compartment_id = "${var.compartment_ocid}"
  vcn_id = "${var.vcn_ocid}"
...
# subnets
resource "oci_core_subnet" "pg_net" {
  compartment_id = "${var.compartment_ocid}"
  vcn_id = "${var.vcn_ocid}"
  display_name = "pg_net"
  availability_domain = "${var.ads[0]}"
  cidr_block = "${var.vcn_subnet_cidrs[0]}"
  route_table_id = "${oci_core_route_table.pg_rt.id}"
  security_list_ids = [ "${oci_core_security_list.pg_sl.id}" ]
